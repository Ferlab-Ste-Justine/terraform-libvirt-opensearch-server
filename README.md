...
- **extra_http_bind_ips**: Extra ip(s) to which the node binds for http communication specifically (not transport communication). If not set, the ip of the first interface from either **libvirt_networks** or **macvtap_interfaces** will still be bound for this purpose.
